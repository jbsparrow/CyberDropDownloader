...
class ProgramUI:
    ...
    def _retry_failed_download(self) -> bool:
        ...
        config.cli.cli_only_args.retry_failed = True
        return True
    ...
    def _change_config(self) -> None:
        ...
        if user_prompts.switch_default_config_to(self.manager, selected_config):
            config.set_default_config(selected_config)
    ...
    def _clear_cache(self) -> None:
        ...
        for url in urls:
            asyncio.run(cache.request_cache.delete_url(url))

        console.print("\nExecuting database vacuum. This may take several minutes, please wait...")
        try:
            vacuum_database(config.appdata.cache_db)
    ...
    def _edit_auth_config(self) -> None:
        self._open_in_text_editor(config.current_config.auth_config_file)

    def _edit_global_config(self) -> None:
        self._open_in_text_editor(config.appdata.global_config_file)

    def _edit_config(self) -> None:
        if self.manager.multiconfig:
            self.print_error("Cannot edit 'ALL' config")
            return
        self._open_in_text_editor(config.current_config.config_file)
    ...
    def _create_new_config(self) -> None:
        ...
        if user_prompts.switch_default_config_to(self.manager, config_name):
            config.set_default_config(config_name)
        config.load_config(config_name)
        self._open_in_text_editor(config.current_config.config_file)

    def _edit_urls(self) -> None:
        self._open_in_text_editor(config.settings.files.input_file, reload_config=False)
    ...
    def _delete_config(self) -> None:
        ...
        selected_config = user_prompts.select_config(configs)
        if selected_config == config.current_config.folder.name:
            self.print_error("You cannot delete the currently active config")
            return

        if config.get_default_config() == selected_config:
    ...
    def _show_simpcity_disclaimer(self) -> None:
        ...
        cache.save("simp_disclaimer_shown", True)

    def _open_in_text_editor(self, file_path: Path, *, reload_config: bool = True) -> None:
        try:
        ...
        if reload_config:
            ...
            config.load_config(config.current_config.folder.name)
    ...
    def _get_changelog(self) -> str | None:
        ...
        path = config.appdata / "CHANGELOG.md"
