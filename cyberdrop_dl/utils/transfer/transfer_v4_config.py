...
def transfer_v4_config(manager: Manager, new_config_name: str, old_config_path: Path) -> None:
    ...
    try:
        ...
        manager.config_manager.save_as_new_config(
            new_settings, ConfigSettings.model_validate(new_user_data.model_dump())
        )
        ...
        logging.getLogger("cyberdrop_dl_startup").error(
            f"Issue while processing v4 transfer of {old_config_path.absolute()} please update/delete the file before trying again"
        )
        logging.getLogger("cyberdrop_dl_startup").error(
            "Please read the documentation for how to handle v4 transition errors: https://script-ware.gitbook.io/cyberdrop-dl/transition-to-v6#importing-previous-configs"
        )
        raise e
