...
class TransitionManager:
    ...
    def check_cache_for_moved(self, cache_file: Path) -> bool:
        ...
        cache = self.manager.yaml_manager.load(cache_file, create=True)
        if not cache:
            cache = {"first_startup_completed": False}
            self.manager.yaml_manager.save(cache_file, cache)
        return bool(cache.get("first_startup_completed", False))

    def set_first_startup_completed(self, cache_file: Path) -> None:
    ...
    def transfer_v4_config(self, old_config_path: Path, new_config_name: str) -> None:
        ...
        old_data = self.manager.yaml_manager.load(old_config_path)
        ...
        self.manager.yaml_manager.save(new_config_path, new_user_data)
        self.manager.yaml_manager.save(new_auth_path, new_auth_data)
        self.manager.yaml_manager.save(new_global_settings_path, new_global_settings_data)
