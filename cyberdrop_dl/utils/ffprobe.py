...
class Stream:
    ...
    def validate(cls, stream_info: Mapping[str, Any]) -> dict[str, Any]:
        ...
        new_attrs = {"tags": tags, "duration": duration, "bitrate": bitrate}
        return info | new_attrs
...
class VideoStream(Stream):
    ...
    def validate(cls, stream_info: StreamDict) -> dict[str, Any]:
        ...
        defaults = super().validate(stream_info)
        new_attrs = {"width": width, "height": height, "fps": fps, "resolution": resolution}
        return defaults | new_attrs
...
class FFprobe:
    ...
    async def probe(self, file_or_url: Path | URL, /, *, headers: Mapping[str, str] | None = None) -> FFprobeResult:
...
async def _run_command(command: Sequence[str]) -> SubProcessResult:
    ...
    log_debug(f"Running ffprobe command: {joined_command}")
