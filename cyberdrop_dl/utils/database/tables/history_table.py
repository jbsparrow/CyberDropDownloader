...
class HistoryTable:
    ...
    async def check_complete(self, domain: str, url: URL, referer: URL) -> bool:
        ...
        result = await cursor.execute(
            """SELECT referer, completed FROM media WHERE domain = ? and url_path = ?""", (domain, url_path)
        )
    ...
    async def check_album(self, domain: str, album_id: str) -> bool | dict[Any, Any]:
        ...
        cursor = await self.db_conn.cursor()
        result = await cursor.execute(
            """SELECT url_path, completed FROM media WHERE domain = ? and album_id = ?""", (domain, album_id)
        )
    ...
    async def check_complete_by_referer(self, domain: str, referer: URL) -> bool:
        ...
        cursor = await self.db_conn.cursor()
        result = await cursor.execute(
            """SELECT completed FROM media WHERE domain = ? and referer = ?""", (domain, str(referer))
        )
        sql_file_check = await result.fetchone()
    ...
    async def insert_incompleted(self, domain: str, media_item: MediaItem) -> None:
        ...
        try:
            ...
            await self.db_conn.execute(
                """DELETE FROM media WHERE domain = 'no_crawler' and url_path = ?""", (url_path,)
            )
        await self.db_conn.execute(
    ...
    async def add_filesize(self, domain: str, media_item: MediaItem) -> None:
        ...
        await self.db_conn.execute(
            """UPDATE media SET file_size=? WHERE domain = ? and url_path = ?""", (file_size, domain, url_path)
        )
        await self.db_conn.commit()
    ...
    async def get_downloaded_filename(self, domain: str, media_item: MediaItem) -> str:
        ...
        cursor = await self.db_conn.cursor()
        result = await cursor.execute(
            """SELECT download_filename FROM media WHERE domain = ? and url_path = ?""", (domain, url_path)
        )
        sql_file_check = await result.fetchone()
    ...
    async def fix_bunkr_v4_entries(self) -> None:
        ...
        for entry in bunkr_entries:
            ...
            entry[0] = "bunkrr"
            await self.db_conn.execute(
                """INSERT or REPLACE INTO media VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)""", entry
            )
        await self.db_conn.commit()
