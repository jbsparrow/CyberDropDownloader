...
def error_handling_wrapper(func):
    ...
    async def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            await self.manager.log_manager.write_scrape_error_log(
                link, " No File Extension")
            await self.manager.progress_manager.scrape_stats_progress.add_failure(
                "No File Extension")
            ...
            await log(f"Scrape Failed: {link} (Invalid Content Type Received)",
                      40)
            await self.manager.log_manager.write_scrape_error_log(
                link, " Invalid Content Type Received")
            await self.manager.progress_manager.scrape_stats_progress.add_failure(
            ...
            if hasattr(e, "status"):
                if hasattr(e, "message"):
                    await log(
                        f"Scrape Failed: {link} ({e.status} - {e.message})",
                        40)
                    await self.manager.log_manager.write_scrape_error_log(
                        link, f" {e.status} - {e.message}")
                else:
...
async def get_filename_and_ext(filename: str,
                               forum: bool = False) -> Tuple[str, str]:
...
async def get_download_path(manager: Manager, scrape_item: ScrapeItem,
                            domain: str) -> Path:
...
async def remove_id(manager: Manager, filename: str,
                    ext: str) -> Tuple[str, str]:
    """Removes the additional string some websites adds to the end of every filename"""
    original_filename = filename
    if manager.config_manager.settings_data["Download_Options"][
...
async def check_partials_and_empty_folders(manager: Manager):
    ...
    if manager.config_manager.settings_data["Runtime_Options"][
    ...
    elif not manager.config_manager.settings_data["Runtime_Options"][
            "skip_check_for_partial_files"]:
        await log_with_color("Checking for partial downloads...", "yellow", 20)
        partial_downloads = any(
            f.is_file()
            for f in manager.path_manager.download_dir.rglob("*.part"))
        if partial_downloads:
            await log_with_color(
                "There are partial downloads in the downloads folder",
                "yellow", 20)
        temp_downloads = any(
            Path(f).is_file()
            for f in await manager.db_manager.temp_table.get_temp_names())
        if temp_downloads:
            await log_with_color(
    ...
    if not manager.config_manager.settings_data["Runtime_Options"][
...
async def check_latest_pypi():
    ...
    if current_version != latest_version:
        await log_with_color(
            f"New version of cyberdrop-dl available: {latest_version}",
            "bold_red", 30)
