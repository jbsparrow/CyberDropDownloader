...
async def get_filename_and_ext(filename: str, forum: bool = False) -> Tuple[str, str]:
    ...
    filename = (
        ...
        if len(filename_parts[0]) > MAX_NAME_LENGTHS["FILE"]
...
async def get_download_path(
    manager: Manager, scrape_item: ScrapeItem, domain: str
) -> Path:
    ...
    if manager.config_manager.settings_data["Runtime_Options"]["delete_partial_files"]:
        ...
        if partial_downloads:
            await log_with_color(
                "There are partial downloads in the downloads folder", "yellow", 20
            )
        temp_downloads = any(
            Path(f).is_file()
...
async def check_latest_pypi():
    ...
    if current_version != latest_version:
        await log_with_color(
            f"New version of cyberdrop-dl available: {latest_version}", "bold_red", 30
        )
