...
class DownloadClient:
    ...
    async def _download(
        ...
        if domain == "pixeldrain":
            if self.manager.config_manager.authentication_data["PixelDrain"]["pixeldrain_api_key"]:
                download_headers["Authorization"] = await self.manager.download_manager.basic_auth(
                    "Cyberdrop-DL", self.manager.config_manager.authentication_data["PixelDrain"]["pixeldrain_api_key"]
                )
        ...
        async with client_session.get(
            download_url, headers=download_headers, ssl=self.client_manager.ssl_context, proxy=self.client_manager.proxy
        ) as resp:
    ...
    async def _append_content(
        self, media_item: MediaItem, content: aiohttp.StreamReader, update_progress: partial
    ) -> None:
