    ...
    async def profile(self, scrape_item: ScrapeItem) -> None:
        ...
        for post in content:
            if "javascript" in post.get("href"):
                ...
                video_link_str: str = video_tag.get("src")
                video_link = URL(video_link_str, encoded="%" in video_link_str)
                new_scrape_item = self.create_scrape_item(scrape_item, video_link, add_parent=scrape_item.url)
                self.handle_external_links(new_scrape_item)
            else:
                link_str: str = post.get("href")
                link = URL(link_str, encodd="%" in link_str)
        ...
        if next_page:
            ...
            new_scrape_item = self.create_scrape_item(scrape_item, URL(next_page_str, encoded="%" in next_page_str))
