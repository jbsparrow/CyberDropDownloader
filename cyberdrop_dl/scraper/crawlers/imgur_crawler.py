...
from cyberdrop_dl.errors import LoginError, ScrapeError
...
class ImgurCrawler(Crawler):
    ...
    async def album(self, scrape_item: ScrapeItem) -> None:
        ...
        async with self.request_limiter:
            ...
            JSON_Obj = await self.client.get_json(self.domain, api_url, with_headers=self.headers, origin=scrape_item)
    ...
    async def check_imgur_credits(self, scrape_item: ScrapeItem | None = None) -> None:
        ...
        credits_obj = await self.client.get_json(
            self.domain, credits_url, with_headers=self.headers, origin=scrape_item
        )
