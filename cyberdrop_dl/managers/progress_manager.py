...
class ProgressManager:
    ...
    async def startup(self) -> None:
        ...
        progress_layout.split_column(
            ...
            Layout(
                renderable=await self.scraping_progress.get_progress(),
                name="Scraping",
                ratio=2,
            ),
            Layout(
                renderable=await self.file_progress.get_progress(),
                name="Downloads",
                ratio=2,
            ),
        )
        progress_layout["upper"].split_row(
            Layout(
                renderable=await self.download_progress.get_progress(),
                name="Files",
                ratio=1,
            ),
            Layout(
                renderable=await self.scrape_stats_progress.get_progress(),
                name="Scrape Failures",
                ratio=1,
            ),
            Layout(
                renderable=await self.download_stats_progress.get_progress(),
    ...
    async def print_stats(self) -> None:
        ...
        await log_with_color(
            f"Skipped By Config {self.download_progress.skipped_files} files",
            "yellow",
            20,
        )
        await log_with_color(
