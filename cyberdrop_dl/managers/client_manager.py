...
class ClientManager:
    ...
    def __init__(self, manager: Manager):
        ...
        self.download_spacer = {'bunkr': 0.5, 'bunkrr': 0.5, 'cyberdrop': 0,
                                'cyberfile': 0, "pixeldrain": 0, "coomer": 0.5, "kemono": 0.5}
    ...
    async def check_http_status(self, response: ClientResponse, download: bool = False) -> None:
        ...
        if "imgur" in response.url.host.lower():
            try:
                ...
                if "status" in JSON_Resp:
                    raise ScrapeFailure(
                        JSON_Resp['status'], JSON_Resp['data']['error'])
            except ContentTypeError:
                pass
        ...
        if not headers.get('Content-Type'):
            raise DownloadFailure(
                status=CustomHTTPStatus.IM_A_TEAPOT, message="No content-type in response header")
