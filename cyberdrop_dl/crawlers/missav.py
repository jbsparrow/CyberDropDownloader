...
class MissAVCrawler(Crawler):
    ...
    async def video(self, scrape_item: ScrapeItem) -> None:
        ...
        segments = get_segments(m3u8_video_content, video_part_base_url, filename)

        await self.handle_hls(m3u8_playlist_url, scrape_item, filename, ext, segments)
...
def get_segments_parts(m3u8_video_content: str) -> Generator[str]:
    for line in m3u8_video_content.splitlines():
        segment = line.strip()
...
def get_segments(m3u8_video_content: str, base_url: URL, filename: str) -> Generator[HlsSegment]:
    # filename should have been pre-sanitized with `get_filename_and_ext`
    last_segment_part = get_last_part(m3u8_video_content)
    last_index_str = re.sub(r"\D", "", last_segment_part)
    padding = max(5, len(last_index_str))
    parts = get_segments_parts(m3u8_video_content)
    for index, part in enumerate(parts, 1):
        url = base_url / part
        name = f"{filename}-{index:0{padding}d}.cdl_hsl"
        yield HlsSegment(part, name, url)


def get_last_part(m3u8_video_content: str) -> str:
    for line in reversed(m3u8_video_content.splitlines()):
